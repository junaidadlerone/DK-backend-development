...
wss.on('connection', (ws) => {
    ...
    ws.on('message', async (message) => {
        try {
            ...
            if (!data.postgrid_api_key) {
                ws.send(JSON.stringify({
                    status: 'error',
                    message: 'Missing postgrid_api_key. Please provide it in the request.'
                }));
                return;
            }
            ...
            if (!data.supabase_anon_key) {
                ws.send(JSON.stringify({
                    status: 'error',
                    message: 'Missing supabase_anon_key. Please provide it in the request.'
                }));
                return;
            }
            ...
            await verifyAddresses(ws, data.zone_id, data.postgrid_api_key, data.supabase_anon_key, showOnlyVerified);
        } catch (error) {
