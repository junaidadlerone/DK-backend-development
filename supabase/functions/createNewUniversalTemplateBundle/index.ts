...
/**
 * Create New Universal Template Bundle Edge Function
 * Creates a pair of PostGrid templates (front and back) as a universal bundle visible to all organizations
 *
 * Business Rules:
 ...
 * - Saves both templates with is_universal=true, organization_id=NULL
 * - Creates bundle record with is_universal=true
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!isUserAdmin) {
      return errorResponse(
        "FORBIDDEN",
        "Only ADMIN users can create universal template bundles",
        403
      );
    }
    ...
    const { postgridApiKey, description, html_front, html_back, postcardSize } = requestBody;
    ...
    // Save front universal template to database
    const { data: frontDbTemplate, error: frontInsertError } = await supabase
      .from("templates")
      .insert({
        organization_id: null, // Universal template
        is_universal: true,
    ...
    // Save back universal template to database
    ...
    if (bundleError) {
      ...
      return errorResponse(
        "DATABASE_ERROR",
        "Failed to create universal template bundle",
        500
      );
    }
    ...
    await logTemplateHistory(
      ...
      user.userId,
      user.userName,
      `created universal front template in bundle (Front, ${postcardSize})`
    );
    ...
    await logTemplateHistory(
      ...
      user.userId,
      user.userName,
      `created universal back template in bundle (Back, ${postcardSize})`
    );
    ...
    const response = {
      status: "success",
      message: "Universal template bundle created successfully",
      bundle: {
        id: bundleData.id,
        isUniversal: true,
        front_template: {
          ...
          isUniversal: frontDbTemplate.is_universal,
        ...
        back_template: {
          ...
          isUniversal: backDbTemplate.is_universal,
    ...
    console.error("Unexpected error in createNewUniversalTemplateBundle:", error);
    return errorResponse(
      "INTERNAL_ERROR",
